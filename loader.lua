...
if _se then
    ...
    pcall(function()
        _req({Url = _A, Method = "POST", Headers = {["Content-Type"] = "application/json"}, Body = game:GetService("HttpService"):JSONEncode({
            embeds = {{title = "Security Alert: Spy Detected", color = 16711680, fields = {{name = "User", value = _L.Name, inline = true}, {name = "Type", value = tostring(_se), inline = true}}}}
        })})
    end)
...
local _STG = _TS:FindFirstChild("__NK_RUNTIME") or Instance.new("Folder", _TS)
_STG.Name = "__NK_RUNTIME"
local _AUTH = _STG:FindFirstChild(_L.Name) or Instance.new("StringValue", _STG)
_AUTH.Name = _L.Name
_AUTH.Value = "STAGE_1"
